...
def test_состояние_сети_уходит_в_телеметрию(monkeypatch):
    ...
    Значение снимается потоком опроса, а не спрашивается у рантайма из
    status(): status() зовёт удар сердца, и вызов, который может не вернуться,
    останавливал узел целиком.
