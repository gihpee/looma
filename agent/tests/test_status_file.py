...
def test_пока_ни_одного_доклада_не_было_уходит_голый():
    """Повторять нечего — но молчать нельзя. Узел, у которого ПЕРВЫЙ же сбор
    не вернулся, раньше не отчитывался никогда, при живом цикле. Голый доклад
    несёт главное: узел жив, спишите его не сейчас."""
    агент = _агент_для_доклада()
    агент._collecting.set()

    доклад = агент._telemetry_or_none()

    assert доклад is not None
    assert доклад.telemetry.node_id == "nv-test"
    assert доклад.telemetry.reported_at_unix_ms > 0
