...
# Запас поверх весов и кэша в ПЛАНЕ: активации шага, буферы связи,
# фрагментация. План считается до подъёма, когда измерить нечего, и число
# грубое. Сколько шаг берёт на самом деле, воркер потом измеряет прогоном
# (`StageWorker.stage_cache_room`) и кэш кладёт в то, что осталось; этот
# запас лишь резервирует место в доле карты, чтобы было куда мерить.
# Занизить его хуже, чем завысить: тогда после замера под кэш не останется
# ничего, и стадия откажет на подъёме.
