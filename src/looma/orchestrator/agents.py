...
class AgentHub:
    ...
    async def request_stream(self, task_id: str, *, method: str = "GET", path: str = "/",
        ...
        Ждём каждый кусок не меньше, чем стадия ждёт свой конвейер
        (`--stage-timeout-s`, 900 с): иначе оркестратор обрывает поток раньше,
        чем стадия успевает сказать, что именно молчит.
